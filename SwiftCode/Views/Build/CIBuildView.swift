...
struct CIBuildView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                LinearGradient(
                    colors: [.blue.opacity(0.3), .purple.opacity(0.35), .black],
    ...
    private var headerCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Dynamic CI Configuration")
                .font(.title3.bold())
                .foregroundStyle(.white)
            Text("Configure build inputs in UI, then generate a fresh build.yml output.")
                .font(.caption)
    ...
    private var configurationCard: some View {
        VStack(spacing: 12) {
            ...
            Picker("Build Configuration", selection: $buildConfiguration) {
                ForEach(AssistCIFunctions.BuildYMLConfig.BuildConfiguration.allCases, id: \.self) { value in
    ...
    private var actionCard: some View {
        VStack(spacing: 10) {
            ...
            Button {
                showStartBuildConfirmation = true
            } label: {
                HStack {
                    if isBuilding {
    ...
    private func startBuild() {
        ...
        if let lastBuildTriggerAt,
           Date().timeIntervalSince(lastBuildTriggerAt) < deduplicationWindow {
            isSuccess = false
        ...
        Task {
            do {
                ...
                _ = try AssistCIFunctions.generateBuildYML(config: buildConfig, workspaceRoot: workspaceRoot)
                ...
                await MainActor.run {
                    ...
                    isSuccess = true
                    statusMessage = "Generated build.yml and updated project workflow."
                    isBuilding = false
                    showStatusAlert = true
                }
            } catch {
